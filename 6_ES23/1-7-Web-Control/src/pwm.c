...
void (*riseFunction) (void);
void (*fallFunction) (void);
char addressSavedPWM = 0;

void pwm_init(int freq, int timerChannel, int ocChannel, int period) {
    callbackAddressOCISR(&functionPWM);
    callbackAddressTimerISR(&functionPWM);
    
    timer_init(freq, timerChannel, period, 1, 7, 3);
...
void retrieveCallback(void (*highFunction)(void), void (*lowFunction)(void))
{
    addressSavedPWM = 1;
    riseFunction = highFunction;
    fallFunction = lowFunction;
}

void functionPWM(char state)
{
    if(addressSavedPWM == 1)
    {
        if(state == 1)
        {
            (*riseFunction) ();
