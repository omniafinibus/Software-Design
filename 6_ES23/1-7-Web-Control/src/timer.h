 ...
 *        
 * @param subPriority - integer
 *        Interrupt sub priority (1-3).
 * 
 * @return success
 */
...
/**
 * get the address of callback function
 * 
 * @param functionAddress - void
 *        address of callback function
 */
void callbackAddressTimerISR(void (*functionAddress)(char));

/**
 * start a interrupt based stopwatch for timer 4
 * 
 * @param status - volatile char address
 *        address of the notify char
 * 
 * @param delayInSec - integer
 *        how many second the stopwatch will wait.
 * 
 */
void startStopWatch3(volatile char * status, int delayInSec);
